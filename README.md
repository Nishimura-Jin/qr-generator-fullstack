...
**タブ切り替え時の入力リセット**
QRコードの種類を切り替えたとき、前の種類で入力していた内容が残ったままだとエラーになることがあったため、切り替えと同時に入力欄をリセットするようにしました。

**トークン切れ時の自動ログアウト**
履歴を取得する際、サーバーからトークンが無効と判断された場合は自動でログアウトするようにしました。そのままエラーが出続けるより、再ログインを促す方がわかりやすいと思ったためです。

**アカウント削除ボタンの配置**
以前はヘッダーにアカウント削除ボタンを置いていましたが、誤操作のリスクを考えてサイドバーの下部に移動しました。履歴を確認しに来た流れの中で目に入る位置にしつつ、メイン画面では見えないようにしています。
